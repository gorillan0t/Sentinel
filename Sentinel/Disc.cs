...
public class Disc : MonoBehaviour
{
    ...
    private bool accessButtonWasPressed;

    private bool accessToggled;
    ...
    private void Update()
    {
        ...
        int  accessMode    = Mathf.Clamp(Cfg.DiscAccess.Value, 0, 2);
        bool accessPressed = AccessButtonPressed(instance2, Mathf.Clamp(Cfg.DiscBtn.Value, 0, 3));
        if (lastAccessMode != accessMode)
        {
            lastAccessMode = accessMode;
            accessToggled  = false;
        }
        else if (accessMode == 2 && accessPressed && !accessButtonWasPressed)
        {
            accessToggled = !accessToggled;
        }

        accessButtonWasPressed = accessPressed;
        bool accessAllowed = accessMode == 0 || accessMode == 1 && accessPressed || accessMode == 2 && accessToggled;
        switch (state)
        {
            case DiscState.Hidden:
                if (Cfg.Palm.Value && accessAllowed && !RingMenu.IsOpen && !PcMenu.IsOpen && !PlayerRay.IsAiming && (accessMode != 0 || Glancing(transform, val, flag)))
                {
            ...
            case DiscState.Palm:
            {
                ...
                if (Cfg.Palm.Value && accessAllowed && !RingMenu.IsOpen && !PcMenu.IsOpen && !PlayerRay.IsAiming && (accessMode != 0 || flag4 || Time.time - lastGlanceAt <= 0.35f))
                {
                    palmHoldTime   += Time.deltaTime;
                    palmVisibility =  Mathf.MoveTowards(palmVisibility, accessMode != 0 || flag4 ? 1f : 0.4f, Time.deltaTime * 5f);
    ...
    private static bool AccessButtonPressed(ControllerInputPoller poller, int button) => button switch
                                                                                         {
                                                                                                 0     => poller.leftControllerPrimaryButton,
                                                                                                 1     => poller.leftControllerSecondaryButton,
                                                                                                 2     => poller.rightControllerPrimaryButton,
                                                                                                 var _ => poller.rightControllerSecondaryButton,
                                                                                         };
