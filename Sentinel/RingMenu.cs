...
public class RingMenu : MonoBehaviour
{
    ...
    private void BuildSettingsPage()
    {
        ...
        AddPagedSetting("wind", "WIND EFFECT", WindEffect.PresetName().ToUpperInvariant(), () => ShowPage(Page.Wind, null));
        ...
        AddBooleanSetting("discenabled", "DISC ENABLED", Cfg.Palm);
        AddPagedSetting("discaccess", "DISC APPEARS", Cfg.DiscAccess.Value switch { 1 => "HOLD BUTTON", 2 => "TOGGLE BUTTON", var _ => "LOOK AT PALM", }, () =>
                                                                                                                                                          {
                                                                                                                                                              Cfg.DiscAccess.Value = (Cfg.DiscAccess.Value + 1) % 3;
                                                                                                                                                              ShowPage(Page.Settings, null);
                                                                                                                                                          });

        if (Cfg.DiscAccess.Value != 0)
            AddPagedSetting("discbutton", "DISC BUTTON", BtnNames[Mathf.Clamp(Cfg.DiscBtn.Value, 0, 3)], () =>
                                                                                                         {
                                                                                                             Cfg.DiscBtn.Value = (Cfg.DiscBtn.Value + 1) % 4;
                                                                                                             ShowPage(Page.Settings, null);
                                                                                                         });
    ...
    private void BuildWindPage()
    {
        AddPageTitle("WIND EFFECT", Theme.Soft);
        BeginPagedGrid(0);
        AddPagedSetting("preset", "PRESET", WindEffect.PresetName().ToUpperInvariant(), () =>
                                                                                        {
                                                                                            WindEffect.CyclePreset();
                                                                                            ShowPage(Page.Wind, null);
                                                                                        });

        AddPagedSetting("direction", "DIRECTION +45", WindEffect.DirectionName(), () =>
                                                                                  {
                                                                                      WindEffect.Rotate(45);
                                                                                      ShowPage(Page.Wind, null);
                                                                                  });

        AddPagedSetting("less", "STRENGTH -10%", WindEffect.StrengthName(), () =>
                                                                            {
                                                                                WindEffect.ChangeStrength(-0.1f);
                                                                                ShowPage(Page.Wind, null);
                                                                            });

        AddPagedSetting("more", "STRENGTH +10%", WindEffect.StrengthName(), () =>
                                                                            {
                                                                                WindEffect.ChangeStrength(0.1f);
                                                                                ShowPage(Page.Wind, null);
                                                                            });

        AddPagedSetting("density", "PARTICLES", WindEffect.DensityName(), () =>
                                                                          {
                                                                              WindEffect.StepDensity(1, true);
                                                                              ShowPage(Page.Wind, null);
                                                                          });

        AddPagedSetting("size", "PARTICLE SIZE", WindEffect.SizeName(), () =>
                                                                        {
                                                                            WindEffect.StepSize(1, true);
                                                                            ShowPage(Page.Wind, null);
                                                                        });

        AddPagedSetting("volume", "WIND VOLUME", WindEffect.VolumeName(), () =>
                                                                          {
                                                                              WindEffect.StepVolume(1, true);
                                                                              ShowPage(Page.Wind, null);
                                                                          });

        AddBooleanSetting("audio",   "WIND AUDIO",   Cfg.WindAudio);
        AddBooleanSetting("fog",     "FOG WISPS",    Cfg.WindFog);
        AddBooleanSetting("foliage", "FOLIAGE SWAY", Cfg.WindFoliage);
        AddBackButton(new Vector3(0f, -0.275f, 0f), () => ShowPage(Page.Settings, null));
    }
    ...
    private void BuildRoomPage()
    {
        ...
        {
            AddPagedTimeScrubber();
            AddPagedSetting("timeforward", "TIME +250", "INCREASE", delegate
                                                                    {
                                                                        GameSettings.AdjustTimeValue(250);
                                                                        ShowPage(Page.Room, null);
                                                                    });

            AddPagedSetting("timeback", "TIME -250", "DECREASE", delegate
                                                                 {
                                                                     GameSettings.AdjustTimeValue(-250);
                                                                     ShowPage(Page.Room, null);
                                                                 });
    ...
    private void AddPagedTimeScrubber()
    {
        int position = pagedSettingCount++;
        pagedItemCount = pagedSettingCount;

        if (position / 10 != activeGridPage) return;
        int      cell   = position % 10;
        Vector3  center = new(cell % 2 == 0 ? -0.16f : 0.16f, 0.205f - cell / 2 * 0.095f, 0f);
        MenuItem item   = AddCard("time", center, "TIME · DRAG 0—10,000", GameSettings.GetTimeValueLabel(), () => GameSettings.AdjustTimeValue(250), 0.3f, 0.085f);
        item.onDrag = offset =>
                      {
                          GameSettings.SetTimeValue(Mathf.RoundToInt(Mathf.Clamp01(offset.x / item.hitWidth + 0.5f) * 10000f));
                          item.valueText.text = GameSettings.GetTimeValueLabel();
                      };
    }
    ...
    private void BuildLiveChatPage()
    {
        ...
        AddCard("floatingchat", new Vector3(-0.17f, -0.205f, 0f), "CHAT DISPLAY", FloatingChat.ModeName(), () =>
                                                                                                           {
                                                                                                               FloatingChat.Cycle();
                                                                                                               ShowPage(Page.Chat, null);
                                                                                                           }, 0.28f, 0.075f);

        AddCard("recenterchat", new Vector3(0.17f, -0.205f, 0f), "RECENTER", "NOW", () =>
                                                                                    {
    ...
    private void HandleLaserInput(GorillaTagger arg1)
    {
        ...
        if (!((Object)(object)val == (Object)null) && !((Object)(object)instance == (Object)null))
        {
            ...
            {
                menuItems[num].onDrag(val6 - new Vector2(menuItems[num].targetPosition.x, menuItems[num].targetPosition.y));
            }
    ...
    private void HandleTouchInput(GorillaTagger arg1)
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            if (num2 >= 0)
            {
                ...
                {
                    menuItems[num2].onDrag(new Vector2(val2.x - menuItems[num2].targetPosition.x, val2.y - menuItems[num2].targetPosition.y));
                }
