...
public static class Cfg
{
    ...
    public static ConfigEntry<int> DiscAccess;

    public static ConfigEntry<int> DiscBtn;
    ...
    public static ConfigEntry<int> ChatHud;

    public static ConfigEntry<int> WindPreset;

    public static ConfigEntry<float> WindStrength;

    public static ConfigEntry<int> WindDirection;

    public static ConfigEntry<bool> WindAudio;

    public static ConfigEntry<bool> WindFog;

    public static ConfigEntry<bool> WindFoliage;

    public static ConfigEntry<float> WindDensity;

    public static ConfigEntry<float> WindSize;
    ...
    public static void Load(ConfigFile config)
    {
        ...
        DiscAccess   = config.Bind("disc",     "access",         0,     "0 show when looking at your palm, 1 show while holding the selected button, 2 toggle with the selected button");
        DiscBtn      = config.Bind("disc",     "button",         2,     "disc access button: 0 X, 1 Y, 2 A, 3 B");
        ...
        ChatHud   = config.Bind("youtube", "floating_chat", 0,  "0 off, 1 front display");

        WindPreset    = config.Bind("wind", "preset",        0,     "0 off, 1 forest, 2 desert, 3 winter, 4 blizzard, 5 sandstorm, 6 leaf storm; local cosmetics only");
        WindStrength  = config.Bind("wind", "strength",      0.35f, "0 calm to 1 storm");
        WindDirection = config.Bind("wind", "direction",     0,     "World heading in degrees: 0 +Z, 90 +X");
        WindAudio     = config.Bind("wind", "audio",         true,  "Play local wind audio");
        WindFog       = config.Bind("wind", "fog",           true,  "Local drifting haze for storm presets");
        WindFoliage   = config.Bind("wind", "foliage",       true,  "Sway nearby supported leaf materials");
        WindDensity   = config.Bind("wind", "particles",     1f,    "Particle amount multiplier, 0.5 to 3");
        WindSize      = config.Bind("wind", "particle_size", 1f,    "Particle size multiplier, 0.75 to 3");
        WindVolume    = config.Bind("wind", "volume",        0.5f,  "Wind audio volume, 0 to 1");
